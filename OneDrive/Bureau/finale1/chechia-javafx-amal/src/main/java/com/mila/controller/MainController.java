...
public class MainController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Traduire les en-têtes des colonnes
        colNationalite.setText(translationService.translateToEnglish("Nationalité"));
        colDuree.setText(translationService.translateToEnglish("Durée"));
        colNom.setText(translationService.translateToEnglish("Nom"));
        colPrenom.setText(translationService.translateToEnglish("Prénom"));
        colAge.setText(translationService.translateToEnglish("Âge"));
        colEmail.setText(translationService.translateToEnglish("Email"));
        nationalityChart.setTitle(translationService.translateToEnglish("Répartition par Nationalité"));
    ...
    private void updateNationalityChart() {
        long tunisienCount = programmesList.stream()
            .filter(p -> p.getNationalite().name().equalsIgnoreCase("TUNISIEN"))
            .count();
        long etrangerCount = programmesList.size() - tunisienCount;
        long totalCount = programmesList.size();
        
        // Calculer les pourcentages
        double tunisienPercentage = (tunisienCount * 100.0) / totalCount;
        double etrangerPercentage = (etrangerCount * 100.0) / totalCount;
        
        // Formater les pourcentages avec 1 décimale
        String tunisienFormattedPercentage = String.format("%.1f%%", tunisienPercentage);
        String etrangerFormattedPercentage = String.format("%.1f%%", etrangerPercentage);
        
        // Ajouter au graphique avec le nombre et le pourcentage
        PieChart.Data tunisienData = new PieChart.Data("Tunisien (" + tunisienCount + " - " + tunisienFormattedPercentage + ")", tunisienCount);
        PieChart.Data etrangerData = new PieChart.Data("Étranger (" + etrangerCount + " - " + etrangerFormattedPercentage + ")", etrangerCount);
        
        nationalityChart.getData().setAll(tunisienData, etrangerData);
        
        // Améliorer la visibilité des étiquettes
        nationalityChart.setLabelsVisible(true);
        nationalityChart.setLabelLineLength(20);
        nationalityChart.setLegendVisible(true);
    }
