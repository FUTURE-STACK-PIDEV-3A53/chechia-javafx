...
public class PostulationService implements IService<Postulation> {
    ...
    public boolean ajouter(Postulation postulation) throws SQLException {
        ...
        try (PreparedStatement ps = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            ...
            // Validation et compression de la lettre de motivation
            String lettreMotivation = postulation.getLettreMotivation();
            if (lettreMotivation != null && lettreMotivation.length() > 65535) { // Limite MySQL TEXT
                throw new SQLException("La lettre de motivation est trop longue (max 65535 caractères)");
            }
            ps.setString(6, lettreMotivation);
            
            // Validation et compression du CV
            byte[] cvData = postulation.getCv();
            ps.setBytes(7, cvData);
            ps.setString(8, "EN_LIGNE");
